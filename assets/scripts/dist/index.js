"use strict";
const orderForm = document.getElementById('order-save');
const btnSave = document.getElementById('btn-save');
const breadRadios = document.getElementsByName('bread');
const ingredientsRadios = document.getElementsByName('ingredients');
const section = document.getElementById("section");
const totalPrice = document.getElementById("totalPrice");
const textMobile = document.getElementById("text-mobile");
const textDesktop = document.getElementById("text-desktop");
const pay = document.getElementById("pay");
sessionStorage.removeItem("orderDescription");
let HNumber = 1;
let totalOrder = 0;
btnSave.addEventListener("click", (e) => {
    let selectedBreadPrice = 0;
    let selectedIngredientPrice = 0;
    breadRadios.forEach((option) => {
        if (option.checked) {
            const priceElement = option.parentElement?.querySelector('.price');
            selectedBreadPrice = parseFloat(priceElement.innerText.replace('R$ ', '').replace(',', '.'));
        }
    });
    ingredientsRadios.forEach((option) => {
        if (option.checked) {
            const priceElement = option.parentElement?.querySelector('.price');
